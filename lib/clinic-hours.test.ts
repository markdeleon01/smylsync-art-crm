...
describe('getClinicBusinessHours', () => {
  it('uses the default 8:00 AM to 5:00 PM hours when no env vars are set', () => {
    delete process.env.CLINIC_HOURS_MONDAY;

    const hours = getClinicBusinessHours();

    expect(hours.monday?.startMinutes).toBe(8 * 60);
    expect(hours.monday?.endMinutes).toBe(17 * 60);
    expect(hours.monday?.label).toBe('8:00 AM - 5:00 PM');
  });
  ...
  it('falls back to defaults for invalid values', () => {
    ...
    expect(hours.friday?.label).toBe('8:00 AM - 5:00 PM');
  });
});
